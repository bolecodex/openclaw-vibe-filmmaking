...
> **API 调用方式**（二选一，由 step-actions 的 `image_model` 或项目配置决定）：
> - **火山方舟 Ark（推荐）**：统一鉴权，与 novel-06 视频同源。
>   ```bash
>   python {workspace}/skills-openclaw/seedream-ark/seedream_ark_api.py generate --prompt PROMPT [--image_url URL] [--model MODEL] [--size SIZE]
>   python {workspace}/skills-openclaw/seedream-ark/seedream_ark_api.py wait --task_id TASK_ID
>   ```
>   完成后从返回的 `content` 中取 `image_url` 或 `image.url`。
> - **MCP/xskill（兼容）**：通过 shell 调用 `xskill_api.py generate` / `get_result`，模型如 `fal-ai/bytedance/seedream/v4.5/edit`。

使用 Seedream 模型将分镜头数据转换为 AI 绘画图像，保持角色和场景的一致性。优先使用火山方舟 Ark（seedream-ark）以与视频链路统一。
...
### 3.4 调用 Seedream（Ark 或 xskill）

**方式 A：火山方舟 Ark（推荐）**

有角色参考图时（图生图）：
```bash
python skills-openclaw/seedream-ark/seedream_ark_api.py generate --prompt "[构建的提示词]" --image_url "[角色参考图URL]" --size "[从 style.yaml → image_sizes.storyboard.preset 映射为 宽x高，如 1280x720]"
# 取返回的 id 再执行 wait
python skills-openclaw/seedream-ark/seedream_ark_api.py wait --task_id TASK_ID
```

无角色参考图时（文生图）：
```bash
python skills-openclaw/seedream-ark/seedream_ark_api.py generate --prompt "[最终 prompt]" --size "[同上]"
python skills-openclaw/seedream-ark/seedream_ark_api.py wait --task_id TASK_ID
```

多角色多图时：将多张参考图描述写进 prompt（如 “Figure 1 是…，Figure 2 是…”），`--image_url` 可只传首张或按模型支持传多图（以 Ark 文档为准）。

**方式 B：MCP 工具 generate（兼容）**
...
```json
{
  ...
  "options": { "image_urls": ["[角色1图片URL]", "[角色2图片URL]"] }
}
```
文生图时用 `fal-ai/bytedance/seedream/v4.5/text-to-image`，同上 `prompt` 与 `image_size`。

> **图片尺寸统一**：所有分镜配图的尺寸从 `style.yaml` 的 `image_sizes.storyboard.preset` 读取，保证全剧一致。Ark 的 `--size` 需为 `宽x高`（如 1024x1024、1280x720）。

**S 级精品剧建议**：`style_base` 与 `negative_prompt` 务必从 `style.yaml` 读取并写进 prompt；可选更大尺寸（如 2K 比例）与更强一致性描述；负面词与质量相关说明写进 SKILL 示例，便于生成稳定画质。
